              ...
              {mode === "destination" ? (
                <>
                  ...
                  {showAutocomplete && places.length > 0 && (
                    <div className="absolute top-full left-0 z-50 mt-1 bg-white dark:bg-card border border-border rounded-2xl shadow-2xl overflow-hidden" style={{ minWidth: "320px", maxHeight: "320px", overflowY: "auto" }}>
                      {(places as any[]).map((place: any, idx: number) => {
                        const types: string[] = place.types || [];
                        const isAirport = types.includes("airport");
                        const isHotel = types.some((t: string) => ["lodging", "hotel", "establishment"].includes(t)) && !isAirport;
                        const isLocality = types.some((t: string) => ["locality", "administrative_area_level_1", "country"].includes(t));
                        const PlaceIcon = isAirport ? Plane : isHotel ? BedDouble : isLocality ? Building2 : MapPin;
                        ...
                        return (
                          <button
                            key={place.placeId}
                            className={`w-full text-left px-4 py-2.5 transition-colors flex items-start gap-3 ${idx === 0 ? "bg-muted/60" : "hover:bg-muted/40"}`}
                            onClick={() => {
                              setDestination(name);
