              ...
              {mode === "destination" ? (
                <>
                  ...
                  {showAutocomplete && places.length > 0 && (
                    <div className="absolute top-full left-0 z-50 mt-1 bg-white dark:bg-card border border-border rounded-2xl shadow-2xl overflow-hidden" style={{ minWidth: "360px", maxHeight: "420px", overflowY: "auto" }}>
                      {(() => {
                        ...
                        const renderItem = (place: any, idx: number, isFirst: boolean) => {
                          ...
                          return (
                            <button
                              key={place.placeId}
                              className={`w-full text-left px-4 py-2.5 transition-colors flex items-start gap-3 ${isFirst ? "bg-muted/60" : "hover:bg-muted/40"}`}
                              onClick={() => {
                                if (place.hotelId) {
                            ...
                            >
                              <div className="w-7 h-7 rounded-full bg-muted flex items-center justify-center shrink-0 mt-0.5">
                                <PlaceIcon className="w-3.5 h-3.5 text-muted-foreground" />
                              </div>
                              <div className="min-w-0">
                                <div className="text-sm truncate">{boldedName}</div>
                        ...
                        return (
                          <>
                            {locationItems.length > 0 && (
                              <>
                                <div className="px-4 py-1.5 text-[10px] font-bold text-muted-foreground uppercase tracking-wider bg-muted/30">Destinations</div>
                                {locationItems.map((place: any, idx: number) => renderItem(place, idx, idx === 0))}
                              </>
                            )}
                            {hotelItems.length > 0 && (
                              <>
                                <div className="px-4 py-1.5 text-[10px] font-bold text-muted-foreground uppercase tracking-wider bg-muted/30 border-t border-border">Hotels</div>
                                {hotelItems.map((place: any, idx: number) => renderItem(place, idx, false))}
                              </>
                            )}
                          </>
                        );
