    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <div className="flex flex-1 flex-col lg:flex-row">
        ...
        <div className="flex-1 flex flex-col px-4 sm:px-8 md:px-16 lg:px-24 py-8 sm:py-10 max-w-2xl w-full mx-auto">
          <Link href="/">
            <button className="flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors mb-6 sm:mb-10" data-testid="button-back">
          ...
          {result ? (
            <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  <h1 className="text-xl sm:text-2xl font-bold">Booking Found</h1>
                  <p className="text-xs sm:text-sm text-muted-foreground">Here are your reservation details.</p>
                </div>
              </div>

              <div className="bg-card border border-border rounded-2xl p-5 sm:p-6 space-y-4 shadow-sm">
                <div>
                  <p className="text-[10px] text-muted-foreground uppercase font-semibold tracking-wider mb-1">Hotel</p>
                ...
                <div className="grid grid-cols-1 xs:grid-cols-2 gap-4">
                  <div>
                    <p className="text-[10px] text-muted-foreground uppercase font-semibold tracking-wider mb-1">Check-in</p>
                  ...
                  <div>
                    <p className="text-[10px] text-muted-foreground uppercase font-semibold tracking-wider mb-1">Check-out</p>
                    <p className="text-sm font-medium flex items-center gap-2">
                      <Calendar className="w-4 h-4 text-primary shrink-0" />
                ...
                <div>
                  <p className="text-[10px] text-muted-foreground uppercase font-semibold tracking-wider mb-1">Room</p>
                ...
                <div className="flex items-center justify-between pt-3 border-t border-border">
                  <div>
                    ...
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase ${result.status === "confirmed" ? "bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400" : "bg-muted text-muted-foreground"}`}>
                      {result.status}
                    </span>
                  </div>
                  <div className="text-right">
                    <p className="text-[10px] text-muted-foreground uppercase font-semibold tracking-wider mb-1">Total</p>
                    <p className="text-lg sm:text-xl font-bold text-foreground">${parseFloat(result.totalPrice).toFixed(2)}</p>
                  </div>
                </div>
              </div>
            ...
            <form onSubmit={handleSubmit} className="animate-in fade-in duration-300">
              <div className="flex items-center gap-3 mb-2">
                ...
                <h1 className="text-xl sm:text-2xl font-bold">Enter Booking Details</h1>
              </div>
              ...
              <div className="space-y-5">
                ...
                {error && (
                  <p className="text-xs sm:text-sm text-destructive bg-destructive/10 px-4 py-3 rounded-xl font-medium" data-testid="text-lookup-error">
                    {error}
                  </p>
                )}
