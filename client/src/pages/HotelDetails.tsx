        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 sm:gap-4 mb-4">
          <div className="flex-1 min-w-0">
            <div className="flex flex-wrap items-center gap-1.5 sm:gap-2 mb-1">
              <h1 className="text-lg sm:text-2xl font-bold font-heading leading-tight">{hotel.name}</h1>
              {starCount && (
            ...
            <p className="flex items-center gap-1.5 text-[10px] sm:text-sm text-muted-foreground">
              <MapPin className="w-3 h-3 sm:w-3.5 sm:h-3.5 shrink-0" />
              <span className="truncate max-w-[180px] xs:max-w-none">{hotel.address}</span>
              <span className="mx-0.5 shrink-0">·</span>
              <button onClick={() => setShowMap(true)} className="underline underline-offset-2 hover:text-foreground transition-colors shrink-0" data-testid="button-show-map">{t("hotel.show_map")}</button>
            </p>
          </div>
          <button
          ...
          >
            <Heart className={`w-3 h-3 sm:w-4 sm:h-4 ${wishlist ? "fill-red-500 text-red-500" : ""}`} />
            <span>{wishlist ? t("hotel.saved") : t("hotel.save")}</span>
          </button>
        </div>
