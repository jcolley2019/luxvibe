...
export default function BookingConfirmation() {
  ...
  const bookMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries({ queryKey: ["/api/bookings"] }); // ADD THIS
    },
    onError: (err: any) => {
      setError(
        err.message || "Booking failed. Please try again or contact support.",
      );
    },
  });
  ...
  useEffect(() => {
    ...
    const resolvedTransactionId =
      transactionId || parsedData.transactionId || paymentIntent;
            ...
            <div className="text-center space-y-3 sm:space-y-4 px-4">
              ...
              <p className="text-muted-foreground text-base sm:text-lg">
                ...
                <span className="font-semibold text-foreground">
                  {bookingData.hotel?.name || checkoutData?.hotelName}
                </span>{" "}
                is all set.
              </p>
            </div>
            ...
            <Card className="border-none shadow-xl overflow-hidden rounded-2xl">
              <div className="bg-primary px-5 py-4 sm:px-6 sm:py-4 flex justify-between items-center text-primary-foreground">
                <div className="flex flex-col">
                  <span className="text-[10px] sm:text-sm font-medium opacity-90 uppercase tracking-wider">
                    Booking ID
                  </span>
                ...
                <div className="text-right">
                  <span className="text-[10px] sm:text-xs opacity-75 block uppercase tracking-wider">
                    Conf. Code
                  </span>
                  <span className="font-bold text-sm sm:text-base">
                    {bookingData.hotelConfirmationCode &&
                    bookingData.hotelConfirmationCode !== "test" ? (
                      bookingData.hotelConfirmationCode
                    ) : (
                        ...
                        <p className="text-sm sm:text-base font-medium">
                          {bookingData.checkin || checkoutData?.checkIn
                            ...
                            : checkoutData?.checkIn
                              ? `${checkoutData.checkIn} - ${checkoutData.checkOut}`
                              : "See booking details"}
                        </p>
                      </div>
                    </div>
                  ...
                  <div className="bg-muted/50 rounded-xl p-4 flex items-center justify-between gap-4">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {checkoutData?.refundableTag === "RFN"
                          ? "Free cancellation available"
                          : "Non-refundable"}
                      </p>
                    </div>
                    <div
                      className={`px-3 py-1 rounded-full text-[10px] sm:text-xs font-bold uppercase shrink-0 ${checkoutData?.refundableTag === "RFN" ? "bg-green-100 text-green-700" : "bg-orange-100 text-orange-700"}`}
                    >
                      {checkoutData?.refundableTag === "RFN"
                        ? "Flexible"
                        : "Strict"}
