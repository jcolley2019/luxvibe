...
Hotel search result cards show LiteAPI facility icons mapped from LiteAPI's `facilityIds` integer array via `FACILITY_ID_MAP` in `server/routes.ts` (60+ entries covering: Free WiFi, Pool, Fitness center, Spa, Restaurant, Parking, Non-smoking rooms, Air conditioning, Wheelchair accessible, Family rooms, Pets allowed, Room service, Business center, EV charging, Airport shuttle, 24-hour front desk, Bar, Sauna, etc.). Deduplication is done via `uniqueFacilities()` helper which wraps the mapped array in a `Set`. Cards with rating ≥ 8.5 and ≥ 700 reviews show a red "High demand for your dates" alert badge. The search `METADATA_LIMIT` is 400 (RATES_BATCH=50) to maximize hotel coverage. Hotels without prices (no availability for selected dates) are included in results and sorted to the end. `distanceFromCenter` (km) is computed via Haversine. Search results include `amenities` field (unique room amenities from batch rates roomTypes). Dynamic page titles via `document.title` in `Home.tsx` and `HotelDetails.tsx`. Filter panel includes: property name, price, popular filters (free cancellation, parking, breakfast included via boardCodes, swimming pool, free WiFi, hotels, apartments), distance from city center, distance from landmarks, stars, guest rating, meal plan, brands, neighborhood, facilities (KEY_FACILITIES: 34 items), room amenities (KEY_ROOM_AMENITIES: 22 items with ROOM_AMENITY_ALIASES for fuzzy matching). **Both desktop sidebar and mobile filter sheet** include Facilities and Room amenities sections with "Show all X" toggle.
